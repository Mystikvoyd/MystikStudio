...
$script:CrashLogPath = Join-Path $FusionRoot "Fusion_error.log"
$script:DebugLogPath = Join-Path $FusionRoot "Fusion.debug.log"
$script:DebugMode    = $false
$script:LoraTesterBuild = "CURRENT_LOGFIX_2_PARSE_SAFE"

$ConfigPath = Resolve-FirstExistingPath -Candidates @(
    (Join-Path $FusionRoot "Fusion.config.json")
) -Fallback (Join-Path $FusionRoot "Fusion.config.json")
...
$PrefsPath = Resolve-FirstExistingPath -Candidates @(
    (Join-Path $FusionRoot "Fusion.prefs.json")
) -Fallback (Join-Path $FusionRoot "Fusion.prefs.json")

$RunLogPath = Resolve-FirstExistingPath -Candidates @(
    (Join-Path $FusionRoot "Fusion.runlog.json")
) -Fallback (Join-Path $FusionRoot "Fusion.runlog.json")
...
$lblDebugPath.Text = "Debug log: Fusion.debug.log"
