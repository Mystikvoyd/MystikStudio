...
$ConfigPath         = Join-Path $DashboardRoot "story-dashboard.config.json"
$SavesPath          = Join-Path $DashboardRoot "story-bible.json"
...
function Get-FemaleChestGuideEntry {
    ...
    $chestPrompt = switch ($ChestIndex) {
        1 { "clothed upper-body proportions: small chest/bust silhouette, size 1 on the shared female chart" }
        2 { "clothed upper-body proportions: small-moderate chest/bust silhouette, size 2 on the shared female chart" }
        3 { "clothed upper-body proportions: moderate chest/bust silhouette, size 3 on the shared female chart" }
        4 { "clothed upper-body proportions: moderate-large chest/bust silhouette, size 4 on the shared female chart" }
        5 { "clothed upper-body proportions: large chest/bust silhouette, size 5 on the shared female chart" }
        6 { "clothed upper-body proportions: very large chest/bust silhouette, size 6 on the shared female chart" }
        7 { "clothed upper-body proportions: very large plus chest/bust silhouette, size 7 on the shared female chart" }
        8 { "clothed upper-body proportions: extra large chest/bust silhouette, size 8 on the shared female chart" }
        9 { "clothed upper-body proportions: extreme chest/bust silhouette, size 9 on the shared female chart" }
        10 { "clothed upper-body proportions: maximum full chest/bust silhouette, size 10 on the shared female chart" }
...
$adjustmentHelp.Location = New-Object System.Drawing.Point(16, 846)
$adjustmentHelp.Size = New-Object System.Drawing.Size(930, 42)
$adjustmentHelp.Anchor = "Left,Right,Bottom"
...
$logBox.Location = New-Object System.Drawing.Point(16, 884)
$logBox.Size = New-Object System.Drawing.Size(930, 32)
...
function Get-SelectedGenerationJob {
    ...
    # When the chest slider is set (1-10):
    #   1. Load BustSize_V0-1.safetensors into the Clothing LoRA slot
    #   2. Inject the trigger word "bust size N" into the style prompt
    #   3. Reduce outfit IPAdapter weight so the LoRA can change the silhouette
    # Strength scales: slider 1=0.45, slider 5=0.65, slider 10=0.85
    $bustLoraFileName = "BustSize_V0-1.safetensors"
    $bustTriggerWord = ""
    $bustOutfitWeightOverride = $null
    ...
    if ($chestSliderValue -ge 1 -and $chestSliderValue -le 10) {
        ...
        $bustTriggerWord = "bust size $chestSliderValue"
        # Reduce outfit IPAdapter weight so LoRA can reshape silhouette
        # Lower values = more LoRA influence on body shape
        $bustOutfitWeightOverride = [math]::Round([math]::Max(0.03, 0.08 - ($chestSliderValue - 1) * (0.005)), 3)
        if ([string]::IsNullOrWhiteSpace($clothingLoraName)) {
            $clothingLoraName = $bustLoraFileName
            $clothingLoraStrength = $bustLoraStrength
        }
        else {
            if ([string]::IsNullOrWhiteSpace($realmLoraName)) {
    ...
    # Inject bust size trigger word if slider is active
    if (-not [string]::IsNullOrWhiteSpace($bustTriggerWord)) {
        [void]$stylePromptList.Add($bustTriggerWord)
    }
    ...
    # If bust LoRA is active, override outfit weight to let LoRA reshape silhouette
    if ($null -ne $bustOutfitWeightOverride) {
        $effectiveOutfitWeight = $bustOutfitWeightOverride
    }
...
$btnGenerate.Add_Click({
    ...
    try {
        ...
        if ($bustSlot -ge 1 -and $bustSlot -le 10) {
            ...
            $bustOW = [math]::Round([math]::Max(0.03, 0.08 - ($bustSlot - 1) * (0.005)), 3)
            Add-Log ("Bust LoRA: BustSize_V0-1 @ {0:N2}, trigger='bust size {1}', outfit weight reduced to {2:N3}." -f $bustStr, $bustSlot, $bustOW)
        }
        [System.Windows.Forms.Application]::DoEvents()
